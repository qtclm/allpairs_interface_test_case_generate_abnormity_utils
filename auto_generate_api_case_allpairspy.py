...
class autoGenerateApiCaseAllpairspy(object):
    ...
    already_generate_api_file_params_details_file_name = os.path.splitext(excel_name_params_field_detail_file_name)[
                                                             0] + '_' + already_generate_api_file_name
    already_generate_api_file_params_obj_file_name = os.path.splitext(excel_name_params_obj_file_name)[
                                                         0] + '_' + already_generate_api_file_name
    ...
    def read_all_cases_data(cls):
        if not os.path.exists(os.path.join(cls.swagger_file_path, cls.excel_name_swagger_api_data_file_name)):
            if os.path.exists(os.path.join(cls.project_root_path, cls.excel_name_swagger_api_data_file_name)):
                ...
                shutil.copyfile(src=os.path.join(cls.project_root_path, cls.excel_name_swagger_api_data_file_name),
                                dst=os.path.join(cls.swagger_file_path, cls.excel_name_swagger_api_data_file_name))
            else:
                raise fileNotFoundError(msg='swagger_api_files不存在,请检查文件名是否填写错误,或者执行swagger_api_parse脚本生成swagger文件')
        cases_data_obj = operationExcel(file_path=cls.swagger_file_path,
                                        file_name=cls.excel_name_swagger_api_data_file_name, opeartion_type='read',
                                        data_only=True)
    ...
    def append_params_key_is_object_to_extra_cases(cls, case_params_key_obj, params_keys, extra_cases,
                                                   filter_func, cases_obj=[], is_append_extra_cases=True,
                                                   params_key=""):
    ...
    def dispose_allpairs_obj_str_to_dict(cls, case_str: str, params_keys, extra_cases, filter_func):
        ...
        def match_params_key_not_object(match_expression, case_str, space_comma):
            ...
            return cls.time_util.recursion_remove_char(str_in=case_params_key_result, space_chars=space_comma,
                                                       remove_type=1)
        ...
        def match_params_key_object_main(match_expression, case_str, params_key, index, space_one, space_comma,
            ...
            case_params_key_result = cls.time_util.recursion_remove_char(str_in=case_params_key_result,
                                                                         space_chars=space_comma,
                                                                         remove_type=1)
    ...
    def generate_all_cases(cls, ordered_dict_obj, params_info):
        ...
        def convert_allpairs_cases_to_cases(params_keys, ordered_dict_obj, cases_obj):
            ...
            if len(params_keys) >= 2:
                cls.append_params_key_is_object_to_extra_cases(case_params_key_obj=ordered_dict_obj,
                                                               params_keys=params_keys, extra_cases=cls.extra_cases,
                                                               filter_func=cls.all_pairs_filter_func,
                                                               cases_obj=cases_obj, is_append_extra_cases=False,
                                                               params_key="")
            else:
        ...
        def return_out_cases(params_info, ordered_dict_obj):
            ...
            out_cases = []
            '''将allparis输出的case字符串转换为dict输出'''
            convert_allpairs_cases_to_cases(params_keys=params_keys, ordered_dict_obj=ordered_dict_obj,
                                            cases_obj=out_cases)
            '''将extra_case数据追加到参数中'''
            append_extra_case_to_case(cases_obj=out_cases, extra_cases=cls.extra_cases,
                                      params_key_list=cls.params_key_list)
            return out_cases
            ...
            if custom_api_enum_data.get(params_obj_type):
                if custom_api_enum_type == 'params_key':
                    ...
                    ordered_dict_obj_path = cls.replace_params_values(params_info=uri_params_path_dict,
                                                                      params_type_enum=custom_api_enum_data[
                                                                          params_obj_type])
    ...
    def check_excel_obj(cls, ex_obj=None, is_params_detail=False):
        if ex_obj is None:
            ...
            ex_obj = operationExcel(file_path=cls.file_path, file_name=excel_name, data_only=False,
                                    opeartion_type='read_write')
    ...
    def read_already_generate_api_file(cls, is_params_details):
        ...
        try:
            with open(os.path.join(cls.file_path, file_name), 'r', encoding='utf-8') as f:
                api_list = f.read()
                api_list_out = eval(api_list) if cls.time_util.isevaluatable_unsafety(str_in=api_list,
                                                                                      is_dynamic_import_module=False) else api_list
            return api_list_out
        except:
    ...
    def create_sheet_and_refresh_switch_sheet(cls, case_info_copy, ex_obj):
        ...
        sheet_name = cls.time_util.check_filename(sheet_name,
                                                  priority_matching_chars=[':', '/', '\\', '?', '*', '[', ']'])
    ...
    @classmethod
    def write_params_filed_detail_to_excel(cls, case_info, params_path, params_body, ex_obj: operationExcel = None,
                                           **kwargs):
        '''将参数字段明细写入到excel'''
    ...
    def batch_write_params_filed_detail_to_excel(cls, case_list=None):
        ...
        for index, params in enumerate(case_list[1:]):
            ...
            sheet_name, ex_obj.data = cls.create_sheet_and_refresh_switch_sheet(case_info_copy=params_copy,
                                                                                ex_obj=ex_obj)
            # 写入excel首行数据(表头)
            cls.write_first_line(ex_obj=ex_obj, ex_obj_data_case_names=old_already_generate_api_list,
                                 sheet_name=sheet_name,
                                 case_info_first_line=[f"{sheet_name}模块所有接口生成的接口参数(包含:path、query、body)明细如下"])
            ...
            if params_body in ({}, [], [{}], None) and params_path in ({}, [], [{}], None):
                ...
                cls.write_params_filed_detail_to_excel(case_info=params_copy, params_path=params_path,
                                                       params_body=params_body, ex_obj=ex_obj,
                                                       **custom_api_enum_data_kwargs)
    ...
    def batch_write_cases_obj_to_excel(cls, case_info=None, case_list=None):
        ...
        for index, case in enumerate(case_list[1:]):
            ...
            sheet_name, ex_obj.data = cls.create_sheet_and_refresh_switch_sheet(case_info_copy=case_info_copy,
                                                                                ex_obj=ex_obj)
            # 写入excel首行数据(表头)
            cls.write_first_line(ex_obj=ex_obj, ex_obj_data_case_names=old_already_generate_api_list,
            ...
            if params_body in ({}, [], [{}], None) and params_path in ({}, [], [{}], None):
                ...
                if cls.custom_api_enum_data.get(uri):
                    ...
                    custom_api_enum_data_kwargs = {}
                cls.write_cases_obj_to_excel(case_info=case_info_copy, params_path=params_path, params_body=params_body,
                                             ex_obj=ex_obj, **custom_api_enum_data_kwargs)
...
if __name__ == "__main__":
    params = {"account": "demo", "pwd": "crmeb.com", "key": "533721295cb06314f4bcaacebc28e3bd", "code": "nbcw",
              ...
              "price2": 0.05, "ip": ""}
    #
    auto_case = autoGenerateApiCaseAllpairspy()
