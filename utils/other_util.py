...
class otherUtil(object):
    ...
    @classmethod
    def recursion_remove_char(cls,str_in,space_chars,remove_type=2):
        '''
        Args:
            str_in:
            space_char: 需要被移除的字符
        ...
        def recursion_remove_char_main(str_in_obj,space_char_obj,remove_type_obj):
            ...
            if remove_type_obj==0:
                if str_in_obj[:len(space_char_obj)]!=space_char_obj:
                    return str_in_obj
                return recursion_remove_char_main(str_in_obj=str_in_obj[len(space_char_obj):],space_char_obj=space_char_obj,remove_type_obj=remove_type_obj)
            elif remove_type_obj==1:
                if str_in_obj[-len(space_char_obj):] != space_char_obj:
                    return str_in_obj
                return recursion_remove_char_main(str_in_obj=str_in_obj[:-len(space_char_obj)], space_char_obj=space_char_obj,remove_type_obj=remove_type_obj)
            else:
                str_in_obj=recursion_remove_char_main(str_in_obj=str_in_obj,space_char_obj=space_char_obj,remove_type_obj=0)
                str_in_obj=recursion_remove_char_main(str_in_obj=str_in_obj,space_char_obj=space_char_obj,remove_type_obj=1)
                return str_in_obj
    ...
    def isevaluatable(cls,str_in,is_dynamic_import_module=False,is_ast_eval=True):
        ...
        try:
            if is_dynamic_import_module:
                ...
                import_module_dict=cls.dynamic_import_module(module_name=str_in)
                # print("import_module_dict:",import_module_dict)
    ...
    @classmethod
    def class_type_str_dispose(cls,str_in):
        str_in = str(str_in)
        str_in_match = re.search('[\'"]{0,}<class\s{1}\S+>[\'"]{0,}', str_in)
        if str_in_match:
    ...
    def dynamic_import_module(cls,module_name):
        ...
        module_name=cls.class_type_str_dispose(module_name)
...
if __name__=="__main__":
    ...
    dict1={'company_name': '', 'device_sn': '', 'extra_info': '', 'password': '', 'product_name': '', 'registry_id': '', 'soft_version': '', 'username': ''}
